...
#include "../../bm_util/v1.0/bm_util.h"
...
void parse_args(int argc, char **argv, struct args_t *args){
    int opt;
    while ((opt = getopt(argc, argv, "hd:s:t:")) != -1) {
        switch (opt) {
			case 'd':
...
int main(int argc, char *argv[]){
	...
	long* dt_ptr;
	pthread_t *threads;
	struct args_t args = { DEFAULT_DELAY, DEFAULT_SAMPLE_SIZE, DEFAULT_NO_THREADS };
	...
	for (thread_no = 0; thread_no < args.no_threads; thread_no++) {
		worker_args[thread_no] = (struct worker_args_t) { args.delay, sample_size_per_thread, thread_no, timestamps + (sample_size_per_thread + 1 ) * thread_no};
