...
gen_plot <- function(no_thread, sample_size, output, overflow, delay, data) {
	...
	lines(data_num_subbuf[[1]]$num_subbuf, data_num_subbuf[[1]]$no_lost_events, type = "c", lty = 4)
	...
	if (length(total_buf_sizes) > 1) {
		for (i in 2:length(total_buf_sizes)) {
			...
			lines(data_num_subbuf[[i]]$num_subbuf, data_num_subbuf[[i]]$no_lost_events, type = "c", lty = 4)
	...
	fracs <- round(100 * yticks / data_num_subbuf[[i]]$no_events_expected[1], digits=2)
	axis(4, at=yticks, labels=fracs)
	mtext("lost events (%)", side = 4, line = 3)
