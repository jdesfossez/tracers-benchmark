...
data <- read.csv(paste(path, "getuid_pthread_ftrace_1.csv", sep =""))
for(i in 2:no_repetitions) {
  data <- rbind(data, read.csv(paste(path, "getuid_pthread_ftrace_", toString(i), ".csv", sep ="")))
}
#print(data)
...
write.csv(data_average, paste(path, "getuid_pthread_ftrace.csv", sep =""), row.names=FALSE, quote=FALSE)
