...
function save_stats {
	...
	echo "${mean[1]},${std[1]},$no_lost_events,$no_events_expected,$total_trace_size,$output,$overflow,$num_subbuf,$total_buf_size,$delay,$no_thread,$sample_size,$no_repetitions" >> ${prog_name}_${tracer_name}.csv
}
...
for t in $(seq 1 $no_repetitions); do
	...
	echo "mean,std,no_lost_events,no_events_expected,trace_size,output,overflow,num_subbuf,total_buf_size,delay,no_thread,sample_size,no_repetitions" > ${prog_name}_${tracer_name}.csv
					...
					for overflow in $overflows; do
						for total_buf_size in $total_buf_sizes; do
							for num_subbuf in $num_subbufs; do
								if [ "$no_thread" -lt "$no_cpus" ]; then
									subbuf_size=$(($total_buf_size / $no_thread / $num_subbuf))
								else
