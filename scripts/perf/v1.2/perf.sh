...
function run {
	echo "sudo $perf_cmd record -e 'syscalls:sys_enter_getuid,syscalls:sys_exit_getuid' $cmd > ${prog_name}.out 2>perf.out"
	sudo $perf_cmd record -e 'syscalls:sys_enter_getuid,syscalls:sys_exit_getuid' $cmd > ${prog_name}.out 2>perf.out
