...
data <- read.csv(paste(path, "getuid_pthread_perf_1.csv", sep =""))
for(i in 2:no_repetitions) {
  data <- rbind(data, read.csv(paste(path, "getuid_pthread_perf_", toString(i), ".csv", sep ="")))
}
#print(data)
...
write.csv(data_average, paste(path, "getuid_pthread_perf.csv", sep =""), row.names=FALSE, quote=FALSE)
