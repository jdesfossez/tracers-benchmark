...
function set_lttng_sample_size {
	sample_size_line=$(grep sample_size < lttng.param)
	sed -i "s/$sample_size_line/sample_size: $sample_size/" lttng.param
	cp lttng.param $lttng_path
}
...
total_buf_size=$(grep total_buf_size < lttng.param | cut -d' ' -f2)
sample_size=$(grep sample_size < lttng.param | cut -d' ' -f2)
...
echo "fraction of lost events: $frac_lost_events +- $sdf,"
echo "trace size: $trace_size Kb +- $sdt."
