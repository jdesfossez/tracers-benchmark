...
print(data_lttng)
pdf(paste(path, "figs/lttng.pdf", sep=""))
...
data_lttng_snapshot <- read.csv(paste(path, "getuid_pthread_lttng_snapshot.csv", sep=""))
print(data_lttng_snapshot)
pdf(paste(path, "figs/lttng_snapshot.pdf", sep=""))
...
data_system_tap <- read.csv(paste(path, "getuid_pthread_system_tap.csv", sep=""))
print(data_system_tap)
pdf(paste(path, "figs/system_tap.pdf", sep=""))
...
data_perf <- read.csv(paste(path, "getuid_pthread_perf.csv",  sep=""))
print(data_perf)
pdf(paste(path, "figs/perf.pdf", sep=""))
plot(data_perf$no_thread,data_perf$mean, xlab="Number of threads", ylab="Mean time per getuid call", main="Perf")
dev.off()

data_ftrace <- read.csv(paste(path, "getuid_pthread_ftrace.csv", sep=""))
print(data_ftrace)
pdf(paste(path, "figs/ftrace.pdf", sep=""))
...
pdf(paste(path, "figs/all.pdf", sep=""))
...
y_range <- c(1000, 5000)
pdf(paste(path, "figs/all_zoom_y.pdf", sep=""))
plot(data_lttng$no_thread, data_lttng$mean, ylim=y_range, pch=1, xlab="number of threads", ylab="mean time per syscall getuid")
par(new=T)
plot(data_lttng_snapshot$no_thread, data_lttng_snapshot$mean, ylim=y_range, axes=F, xlab="", ylab="", pch=2)
par(new=T)
